/**
 * Centered RimWorld-style fuel filter overlay for settlement campfires.
 */
class FuelFilterPanel {
    constructor(scene) {
        this.scene = scene;
        this.visible = false;
        this.settle = null;
    ...
    _FF() {
        return typeof FuelFilter !== "undefined" ? FuelFilter : null;
    }
    ...
    _build() {
        ...
        this.bg = scene.add.rectangle(0, 0, 440, 400, 0x120e0a, 0.96)
            ...
            .setOrigin(0, 0)
            .setInteractive();
        this.title = scene.add.text(0, 0, "Fuel", {
        ...
        this.alwaysBtn = this._btn(0, 0, "Always on", () => this._toggleFlag("alwaysOn"), {
            w: 148, h: 22, check: true
        });
        this.preferBtn = this._btn(0, 0, "Prefer logs", () => this._toggleFlag("preferLogs"), {
            w: 148, h: 22, check: true
        });
        ...
        this.root.add([
            ...
            this.alwaysBtn, this.preferBtn, this.closeBtn
        ]);
    }
    ...
    _filter() {
        const FF = this._FF();
        return FF ? FF.normalize(this.thing?.entry?.fuelFilter) : { alwaysOn: true, preferLogs: true };
    }

    _tree() {
        const FF = this._FF();
        const list = this.scene.items?.() || [];
        return FF ? FF.buildTree(list) : [];
    }
    ...
    _commit(next) {
        ...
        if (FF) FF.applyToEntry(this.thing.entry, next);
        else this.thing.entry.fuelFilter = next;
        const settle = this.settle;
        this.scene.settlementSys?.sendNet("setFuelFilter", {
            settlementId: settle?.id,
            uid: this.thing.entry.uid,
            filter: this.thing.entry.fuelFilter || null
        });
        this.refresh();
    }
    ...
    _paintFlags() {
        const f = this._filter();
        if (this.alwaysBtn) {
            this.alwaysBtn._checked = !!f.alwaysOn;
            this.alwaysBtn._paint?.();
        }
        if (this.preferBtn) {
            this.preferBtn._checked = !!f.preferLogs;
            this.preferBtn._paint?.();
        }
    }
    ...
    close() {
        if (this.scene._isUnderFuelFilterPanel?.(this.scene._tooltipTarget)) {
    ...
    layout() {
        ...
        const checkW = Math.round(148 * s);
        const checkH = Math.round(22 * s);
        const checkY = Math.round(42 * s);
        const checkGap = Math.round(8 * s);
        this.alwaysBtn.setPosition(pad + checkW / 2, checkY);
        this.preferBtn.setPosition(pad + checkW + checkGap + checkW / 2, checkY);
        this._sizeBtn(this.alwaysBtn, checkW, checkH, 12);
        this._sizeBtn(this.preferBtn, checkW, checkH, 12);
        const sw = typeof pixelUiStroke === "function" ? pixelUiStroke(s) : 2;
        this.bg.setStrokeStyle(sw, 0x2a2218);
        this._bodyX = pad;
        this._bodyY = checkY + Math.ceil(checkH / 2) + Math.ceil(sw / 2) + Math.round(8 * s);
        this._viewW = w - pad * 2;
    ...
    refresh() {
        ...
        const s = this.scene.uiScale || 1;
        const name = this.thing?.meta?.name || this.thing?.entry?.id || "Campfire";
    ...
    _fillTree(nodes, depth, y, sc, filter, root) {
        ...
        for (const node of nodes || []) {
            const state = FF ? FF.categoryState(filter, tree, node.id) : "on";
            ...
            this._row(rowX, y, rowW, rowH, state, (pointer) => {
                ...
                if (!FF) return;
                this._commit(FF.toggleCategory(this._filter(), tree, node.id));
            });
            if (hasKids) {
            ...
            for (const it of node.items || []) {
                const on = FF ? FF.itemState(filter, tree, it.id) === "on" : true;
                ...
                this._row(itemX, y, itemW, rowH, itemState, () => {
                    if (!FF) return;
                    this._commit(FF.toggleItem(this._filter(), tree, it.id));
